...
//         await mint(wallet.address, minTick, maxTick, expandTo18Decimals(2n))
...
//         ;({ swapExact0For1, pool, mint, swapToHigherPrice, swapToLowerPrice } = await gasTestFixture())
//       })
...
//           await snapshotGasCost(swapExact0For1(expandTo18Decimals(1n) / 10000n, await wallet.address))
//           expect((await pool.slot0()).tick).to.eq(startingTick - 1)
//         })

//         it('second swap in block with no tick movement', async () => {
//           await swapExact0For1(expandTo18Decimals(1n) / 10000n, await wallet.address)
//           expect((await pool.slot0()).tick).to.eq(startingTick - 1)
//           await snapshotGasCost(swapExact0For1(2000, wallet.address))
//           expect((await pool.slot0()).tick).to.eq(startingTick - 1)
//         })
...
//           expect((await pool.slot0()).tick).to.eq(startingTick)
//           await snapshotGasCost(swapExact0For1(expandTo18Decimals(1n) / 10000n, wallet.address))
//           expect((await pool.slot0()).tick).to.eq(startingTick - 1)
//         })

//         it('first swap in block, large swap, no initialized crossings', async () => {
//           await snapshotGasCost(swapExact0For1(expandTo18Decimals(10n), wallet.address))
//           expect((await pool.slot0()).tick).to.eq(-35787)
//         })

//         it('first swap in block, large swap crossing several initialized ticks', async () => {
//           await mint(wallet.address, startingTick - 3 * tickSpacing, startingTick - tickSpacing, expandTo18Decimals(1n))
//           await mint(
//             wallet.address,
...
//           )
//           expect((await pool.slot0()).tick).to.eq(startingTick)
//           await snapshotGasCost(swapExact0For1(expandTo18Decimals(1n), wallet.address))
...
//           await mint(wallet.address, minTick, startingTick - 2 * tickSpacing, expandTo18Decimals(1n))
//           await snapshotGasCost(swapExact0For1(expandTo18Decimals(1n), wallet.address))
...
//             expandTo18Decimals(1n)
//           )
//           await swapExact0For1(expandTo18Decimals(1n) / 10000n, wallet.address)
//           await snapshotGasCost(swapExact0For1(expandTo18Decimals(1n), wallet.address))
...
//               await snapshotGasCost(mint(wallet.address, tickLower, tickUpper, expandTo18Decimals(1n)))
//             })
//             it('add to position existing', async () => {
//               await mint(wallet.address, tickLower, tickUpper, expandTo18Decimals(1n))
//               await snapshotGasCost(mint(wallet.address, tickLower, tickUpper, expandTo18Decimals(1n)))
//             })
//             it('second position in same range', async () => {
//               await mint(wallet.address, tickLower, tickUpper, expandTo18Decimals(1n))
//               await snapshotGasCost(mint(other.address, tickLower, tickUpper, expandTo18Decimals(1n)))
//             })
...
//               await snapshotGasCost(pool.burn(tickLower, tickUpper, expandTo18Decimals(1n)))
//             })
//             it('partial position burn', async () => {
//               await snapshotGasCost(pool.burn(tickLower, tickUpper, expandTo18Decimals(1n) / 2n))
//             })
//             it('entire position burn but other positions are using the ticks', async () => {
//               await mint(other.address, tickLower, tickUpper, expandTo18Decimals(1n))
//               await snapshotGasCost(pool.burn(tickLower, tickUpper, expandTo18Decimals(1n)))
//             })
...
//         it('best case', async () => {
//           await mint(wallet.address, tickLower, tickUpper, expandTo18Decimals(1n))
//           await swapExact0For1(expandTo18Decimals(1n) / 100n, wallet.address)
//           await pool.burn(tickLower, tickUpper, 0n)
//           await swapExact0For1(expandTo18Decimals(1n) / 100n, wallet.address)
//           await snapshotGasCost(pool.burn(tickLower, tickUpper, 0n))
//         })
//       })
...
//           await mint(wallet.address, tickLower, tickUpper, expandTo18Decimals(1n))
//           await swapExact0For1(expandTo18Decimals(1n) / 100n, wallet.address)
//           await pool.burn(tickLower, tickUpper, 0n) // poke to accumulate fees
...
//           await snapshotGasCost(pool.snapshotCumulativesInside(minTick, maxTick))
//         })
//         it('tick above', async () => {
